@extends('layouts.master')
@section('content')

<div class="container-fluid">
    <!-- .Row -->
    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <h3 class="box-title">HOW TO DELETE A PARENT</h3>
                <div class="row">
                <p>The following are the steps to create/add a parent(s):</p>
                    
                    <p> On the Sidebar, select "Users"</p>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/users/all1.png') ?>" width="120%" height="100%"/>
                        </div>
                </div>

                <div class="row">
                <p>For mobile phones: Press on menu</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/users/all2.png') ?>" width="40%" height="60%"/>
                    </div>
                </div>
            </div>
        </div>
    </div>

<div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                <p>Click on “Parent”</p>
                    <div class="row">
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/users/parent/parent_delete4.png') ?>" width="120%" height="100%"/>
                        </div>
                        
                    </div>

                   <p>For mobile phones: Click on "Users" then, "Parents"</p>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/users/parent/parent_delete3.png') ?>" width="40%" height="60%"/>
                        </div>
                </div>
                
            </div>
        </div>
    </div>

    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                   
                    
                       <p>Click on down arrow to view a list of classes</p>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/users/parent/parent_delete5.png') ?>" width="120%" height="100%"/>
                        </div>
                    
                </div>
                <div class="row">
                    <p>For mobile phones:</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/users/parent/parent_delete6.png') ?>" width="40%" height="60%"/>
                    </div>

                </div>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                  
                    
                       <p>You can select a class or select "All" to view all parents registered in the system</p>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/users/parent/parent_delete7.png') ?>" width="120%" height="100%"/>
                        </div>
                    
                </div>
                <div class="row">
                    <p>You can delete the parent profile in the following ways:</p>
                   <p>i. Click delete on the respective parent profile</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/users/parent/parent_delete8.png') ?>" width="120%" height="100%"/>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                    <ul>
                       <p>For mobile phones:</p>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/users/parent/parent_delete9.png') ?>" width="40%" height="60%"/>
                        </div>
                    </ul>
                </div>
                <div class="row">
                    <p>ii. Viewing the profile details before deleting</p>
                   <p>Click on “View” on a respective parent profile</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/users/parent/parent_delete10.png') ?>" width="120%" height="100%"/>
                    </div>

                </div>
            </div>
        </div>
    </div>

    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                
                    
                       <p>For mobile phones:</p>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/users/parent/parent_delete11.png') ?>" width="40%" height="60%"/>
                        </div>
                    
                </div>
                <div class="row">
                    <p>Review the profile and click on “Delete”</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/users/parent/parent_delete12.png') ?>" width="120%" height="100%"/>
                    </div>

                </div>
            </div>
        </div>
    </div>


<div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                  
                    
                       <p>Select the reason for deleting the profile example; “Mistakenly Registered”</p>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/users/parent/parent_delete13.png') ?>" width="120%" height="100%"/>
                        </div>
                    
                </div>
                <div class="row">
                    <p>For mobile phones:</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/users/parent/parent_delete14.png') ?>" width="40%" height="60%"/>
                    </div>

                </div>
            </div>
        </div>
    </div>

    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                    
                    
                       <p>Then Click on “Submit change”</p>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/users/parent/parent_delete15.png') ?>" width="120%" height="100%"/>
                        </div>
                    
                </div>
              
                <div class="row">
                    <p>For mobile phones:</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/users/parent/parent_delete16.png') ?>" width="40%" height="60%"/>
                        <p></p>
                        <p class="alert alert-info">Note: A success notification will appear at the top-right corner, otherwise repeat all the procedures carefully.</p>
                    </div>

                </div>
</div>

@endsection