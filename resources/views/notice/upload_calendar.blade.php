@extends('layouts.master')
@section('content')

<div class="container-fluid">
    <!-- .Row -->
    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <h3 class="box-title">HOW TO UPLOAD SCHOOL CALENDER IN SHULESOFT</h3>
                <div class="row">
                <p>The following are the steps to upload school calendar:</p>
                    

                   
                   <p>On the Sidebar, select "Notice"</p>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/notice/upload/1.png') ?>" width="120%" height="100%"/>
                        </div>
                   
                </div>
                <div class="row">
                    
                    <p>For mobile phones: Press on menu</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/notice/upload/2.png') ?>" width="40%" height="60%"/>
                    </div>

                </div>
            </div>
        </div>
    </div>

<div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                    
                    <p>(i)Click:“Notice”</p>


                        <div class="col-sm-8">
                            <img src="<?= url('public/images/notice/upload/3.png') ?>" width="40%" height="60%"/>
                        </div>
                    
                </div>
               
                <p>Click on “+Add a Notice”</p>
                <div class="row">
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/notice/upload/4.png') ?>" width="120%" height="100%"/>
                    </div>
                   
                </div>
            </div>
        </div>
    </div>

    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                   
                   
                        <p>For mobile phones: </p>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/notice/upload/5.png') ?>" width="40%" height="50%"/>
                        </div>
                   
                </div>
                <div class="row">
                    
                    <p>The school calendar can be uploaded by excel, you can download the sample file</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/notice/upload/6.png') ?>" width="120%" height="90%"/>
                    </div>

                </div>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                  
                   
                        <p>For mobile phones: </p>
                        
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/notice/upload/7.png') ?>" width="40%" height="50%"/>
                        </div>
                   
                </div>
                <div class="row">
                    
                    <p>A school calendar sample file</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/notice/upload/8.png') ?>" width="120%" height="90%"/>
                    </div>
                    
                </div>
            </div>
        </div>
    </div>

    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                    
                        <p>Click on “browse” to upload your file</p>


                        <div class="col-sm-8">
                            <img src="<?= url('public/images/notice/upload/9.png') ?>" width="120%" height="100%"/>
                        </div>
                    
                </div>
                <div class="row">
                    
                    <p>For mobile phones:</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/notice/upload/10.png') ?>" width="40%" height="50%"/>
                    </div>

                </div>
            </div>
        </div>
    </div>

    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                    
                        <p>Click on “Submit” </p>

	                          

                        <div class="col-sm-8">
                            <img src="<?= url('public/images/notice/upload/11.png') ?>" width="120%" height="100%"/>
                        </div>
                    
                </div>
                <div class="row">
                    
                    <p>For mobile phones:</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/notice/upload/12.png') ?>" width="40%" height="50%"/>
                    </div>

                </div>
            </div>
        </div>
    </div>
    
    
      <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                    
                    <p class= "alert alert-info">Note: A success notification will appear at the top-right corner, otherwise repeat all the procedures carefully.</p>

	                       

                      
                    
                </div>
               
            </div>
        </div>
    </div>



@endsection