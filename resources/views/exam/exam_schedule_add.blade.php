@extends('layouts.master')
@section('content')

<div class="container-fluid">
    <!-- .Row -->
    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <h3 class="box-title">HOW TO DEFINE EXAM SCHEDULE</h3>
                <div class="row">
                <p class= "alert alert-info"><b>Important Note: Before creating/defining exam schedules,you should have already defined class, class level, subject, section, exam and academic year.</b></p>
                <p>The following are the steps to define exam schedules:</p>
                    

                    
                    <p>On the sidebar, Select “Exam”</p>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/exam/exam schedule/add/1.png') ?>" width="120%" height="100%"/>
                        </div>
                    
                </div>
                <div class="row">
                    
                        <p>For mobile phones: Press on menu</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/exam/exam schedule/add/2.png') ?>" width="40%" height="60%"/>
                    </div>

                </div>
            </div>
        </div>
    </div>

<div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                    <p>For mobile phones: </p>
                    <p>	(i)Click:Exam then,<br>
                (ii)Schedule</p>


                        <div class="col-sm-8">
                            <img src="<?= url('public/images/exam/exam schedule/add/3.png') ?>" width="40%" height="60%"/>
                        </div>
                    
                </div>
                
                <p>Click on "schedule"</p>
                <div class="row">
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/exam/exam schedule/add/4.png') ?>" width="120%" height="100%"/>
                    </div>
                    
                </div>
            </div>
        </div>
    </div>

    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                   
                    
                        <p>Click on "Add Exam Schedule"</p>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/exam/exam schedule/add/5.png') ?>" width="120%" height="100%"/>
                        </div>
                    
                </div>
                <div class="row">
                    
                    <p>For mobile phones:</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/exam/exam schedule/add/6.png') ?>" width="40%" height="60%"/>
                    </div>

                </div>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                  
                    
                        <p>Enter all the required information</p>
                        
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/exam/exam schedule/add/7.png') ?>" width="120%" height="100%"/>
                        </div>
                    
                </div>
                <div class="row">
                    
                    <p>For mobile phones:</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/exam/exam schedule/add/8.png') ?>" width="40%" height="60%"/>
                    </div>
                    
                </div>
            </div>
        </div>
    </div>

    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                    <ul>
                        <p>Click on "Add Exam Schedule"</p>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/exam/exam schedule/add/9.png') ?>" width="120%" height="100%"/>
                        </div>
                    </ul>
                </div>
                <div class="row">
                    
                    <p>For mobile phones:</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/exam/exam schedule/add/10.png') ?>" width="40%" height="60%"/>
                    </div>

                </div>
            </div>
        </div>
    </div>

    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                
                    <p></p>
                <p class= "alert alert-info">Note: A success notification will appear at the top-right corner, otherwise repeat all the procedures carefully.</p>
                        
                    
                </div>
               
            </div>
        </div>
    </div>




@endsection