@extends('layouts.master')
@section('content')

<div class="container-fluid">
    <!-- .Row -->
    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <h3 class="box-title">HOW TO ADD A CLASS LEVEL</h3>
                <div class="row">
                    <p>In ShuleSoft System you can create Class Level after Login and do the following steps:</p>
                    <ol>
                        <li>On Sidebar Click on Class Level</li>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/classlevel/classlevel_add1.png') ?>" width="80%" height="80%"/>
                        </div>
                    </ol>
                </div>
                <div class="row">
                    <p>For Mobile, Press Menu First</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/classlevel/classlevel_add2.png') ?>" width="80%" height="80%"/>
                    </div>

                </div>
            </div>
        </div>
    </div>

<div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                    <p>Then Click Class Level</p>

                        <div class="col-sm-8">
                            <img src="<?= url('public/images/classlevel/classlevel_add3.png') ?>" width="80%" height="80%"/>
                        </div>
                </div>
                <ol>
                <li>Then click on Add Level</li>
                <div class="row">
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/classlevel/classlevel_add4.png') ?>" width="80%" height="80%"/>
                    </div>
                    </ol>
                </div>
            </div>
        </div>
    </div>

    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                   
                    <ol>
                        <li>For Mobile</li>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/classlevel/classlevel_add5.png') ?>" width="80%" height="80%"/>
                        </div>
                    </ol>
                </div>
                <div class="row">
                    <p>Then Enter all required Information in the fields for Class level definition</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/classlevel/classlevel_add6.png') ?>" width="80%" height="80%"/>
                    </div>

                </div>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                  
                    <ol>
                        <li>For Mobile</li>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/classlevel/classlevel_add7.png') ?>" width="80%" height="80%"/>
                        </div>
                    </ol>
                </div>
                <div class="row">
                    <p>Then Click on Add Level</p>
                    <div class="col-sm-8">
                        <img src="<?= url('public/images/classlevel/classlevel_add8.png') ?>" width="80%" height="80%"/>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="row">
        <div class="col-md-12 col-sm-12 col-lg-12">
            <div class="white-box">
                <div class="row">
                    <ul>
                        <li>For Mobile</li>
                        <div class="col-sm-8">
                            <img src="<?= url('public/images/classlevel/classlevel_add9.png') ?>" width="80%" height="80%"/>
                        <p>Note: If the all procedure is done well and all text fields is full of required information, the success notification will appear on top at the right corner, otherwise repeat all the procedures carefully.</p>
                        </div>
                    </ul>
                </div>
            </div>
        </div>
    </div>

</div>

@endsection